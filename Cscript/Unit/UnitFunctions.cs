...
public partial class Unit
{
    ...
    public float TakeBulletDamage(float amount, Unit user, Skill skill)
    {
        ...
        {
            // 擦弹检定
            if (GD.Randf() < MathEx.Logistic(1-0.7f*(CurrentSp / MaxSp), Ua.BulletGraze - user.Ua.BulletDamageAccuracy))
            {
                CurrentSp += amount;
                if (CurrentSp > MaxSp)
                {
    ...
    public void TakeBodyDamage(float amount, Unit user, Skill skill)
    {
        ...
        foreach (var status in Status.ToList())
    ...
    public void LearnSkill(Skill skill)
    {
        ...
        if (skill.SkillGroup == "Item" && this == Player.PlayerUnit)
...
当前状态:";
    ...
    {
        var lag = GridInVision().Where(g => g.unit != null && g.unit.friendness * friendness < 0).ToArray();
        if (lag.Length > 0)
        {
